...
public class GenEntityTool_Mysql_MybatisSQL {
    public static void main(String[] args) {
        // MYSQL 数据库连接驱动信息
        String databaseName = "lol";// mysql数据库名称
        String user = "root"; // 数据库连接用户名
        String password = "root"; // 数据库连接密码
        String tableName = "user"; // 要生成实体类的数据库表名（数据库中表名称）
    ...
    private void processAllMethod(StringBuffer sb) {
        ...
        for (int i = 0; i < colnames.length; i++) {
            ...
            objGetStr+=objName+".get"+initcap(colnames[i])+"(),";
        }
        objGetStr=objGetStr.substring(0,objGetStr.length()-1);
        System.out.println("\r\n"+objGetStr+"\r\n");
